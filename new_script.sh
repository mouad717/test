...
echo  "## install all extra packages" >> text.txt
GREENTXT "SYSTEM PACKAGES INSTALLATION. PLEASE WAIT"
echo  "SYSTEM PACKAGES INSTALLATION. PLEASE WAIT" >> text.txt
yum -q -y install ${REPO_FAN} >/dev/null 2>&1
echo  "yum -q -y install ${REPO_FAN} >/dev/null 2>&1" >> text.txt
sed -i '0,/gpgkey/s//includepkgs=curl libmetalink libpsl libcurl libssh2\n&/' /etc/yum.repos.d/city-fan.org.repo
echo  "sed -i \'0,/gpgkey/s//includepkgs=curl libmetalink libpsl libcurl libssh2\n&/\' /etc/yum.repos.d/city-fan.org.repo" >> text.txt
yum -q -y install ${EXTRA_PACKAGES} ${PERL_MODULES[@]/#/perl-} >/dev/null 2>&1
echo  "yum -q -y install ${EXTRA_PACKAGES} ${PERL_MODULES[@]/#/perl-} >/dev/null 2>&1" >> text.txt
echo
GREENTXT "CHECKING UPDATES. PLEASE WAIT"
echo  "CHECKING UPDATES. PLEASE WAIT" >> text.txt
## checking updates
...
GREENTXT "THE UPDATES ARE BEING INSTALLED"
			...
			echo  "yum -y -q update >/dev/null 2>&1" >> text.txt
            stop_progress "$pid"
            echo
...
echo "-------------------------------------------------------------------------------------"
echo  "| START THE INSTALLATION OF REPOSITORIES AND PACKAGES |" >> text.txt
echo
WHITETXT "============================================================================="
echo
echo -n "---> Start Percona repository and Percona database installation? [y/n][n]:"
echo  "---> Start Percona repository and Percona database installation? [y/n][n]:" >> text.txt
...
if [ "${repo_percona_install}" == "y" ];then
          echo
			...
			echo  "rpm --quiet -U ${REPO_PERCONA} >/dev/null 2>&1" >> text.txt
            stop_progress "$pid"
            rpm  --quiet -q percona-release
			echo  "rpm  --quiet -q percona-release" >> text.txt
        ...
        then
          echo
            GREENTXT "REPOSITORY HAS BEEN INSTALLED  -  OK"
			  ...
			  echo  "Installation of Percona 5.6 database:" >> text.txt
              echo
              echo -n "     PROCESSING  "
              long_progress &
              pid="$!"
              yum -y -q install ${PERCONA_PACKAGES[@]/#/Percona-Server-}  >/dev/null 2>&1
			  echo  "yum -y -q install ${PERCONA_PACKAGES[@]/#/Percona-Server-}  >/dev/null 2>&1" >> text.txt
              stop_progress "$pid"
              rpm  --quiet -q ${PERCONA_PACKAGES[@]/#/Percona-Server-}
			  echo  "rpm  --quiet -q ${PERCONA_PACKAGES[@]/#/Percona-Server-}" >> text.txt
			  
        if [ "$?" = 0 ] # if package installed then configure
          then
            echo
              GREENTXT "DATABASE HAS BEEN INSTALLED  -  OK"
			  echo  "DATABASE HAS BEEN INSTALLED  -  OK" >> text.txt
              echo
              ## plug in service status alert
			  echo  "## plug in service status alert" >> text.txt
              cp /usr/lib/systemd/system/mysqld.service /etc/systemd/system/mysqld.service
			  echo  "cp /usr/lib/systemd/system/mysqld.service /etc/systemd/system/mysqld.service" >> text.txt
              sed -i "s/^Restart=always/Restart=on-failure/" /etc/systemd/system/mysqld.service
			  echo  "sed -i \"s/^Restart=always/Restart=on-failure/\" /etc/systemd/system/mysqld.service" >> text.txt
              sed -i "s/^Restart=always/Restart=on-failure/" /etc/systemd/system/mysql.service
			  echo  "sed -i \"s/^Restart=always/Restart=on-failure/\" /etc/systemd/system/mysql.service" >> text.txt
              sed -i "/^After.*/a OnFailure=service-status-mail@%n.service" /etc/systemd/system/mysqld.service
			  echo  "sed -i \"/^After.*/a OnFailure=service-status-mail@%n.service\" /etc/systemd/system/mysqld.service" >> text.txt
              sed -i "/^After.*/a OnFailure=service-status-mail@%n.service" /etc/systemd/system/mysql.service
			  echo  "sed -i \"/^After.*/a OnFailure=service-status-mail@%n.service\" /etc/systemd/system/mysql.service" >> text.txt
              sed -i "/Restart=on-failure/a RestartSec=10" /etc/systemd/system/mysqld.service
			  echo  "sed -i \"/Restart=on-failure/a RestartSec=10\" /etc/systemd/system/mysqld.service" >> text.txt
              sed -i "/Restart=on-failure/a RestartSec=10" /etc/systemd/system/mysql.service
			  echo  "sed -i \"/Restart=on-failure/a RestartSec=10\" /etc/systemd/system/mysql.service" >> text.txt
              systemctl daemon-reload
			  echo  "systemctl daemon-reload" >> text.txt
              systemctl enable mysql >/dev/null 2>&1
			  echo  "systemctl enable mysql >/dev/null 2>&1" >> text.txt
			  
              echo
              WHITETXT "Downloading my.cnf file from MagenX Github repository"
			  echo  "Downloading my.cnf file from MagenX Github repository" >> text.txt
              wget -qO /etc/my.cnf https://raw.githubusercontent.com/magenx/magento-mysql/master/my.cnf/my.cnf
			  echo  "wget -qO /etc/my.cnf https://raw.githubusercontent.com/magenx/magento-mysql/master/my.cnf/my.cnf" >> text.txt
			  
              echo
                echo
                 WHITETXT "We need to correct your innodb_buffer_pool_size"
				 echo  "We need to correct your innodb_buffer_pool_size" >> text.txt
                 rpm -qa | grep -qw bc || yum -q -y install bc >/dev/null 2>&1
				 echo  "rpm -qa | grep -qw bc || yum -q -y install bc >/dev/null 2>&1" >> text.txt
                 IBPS=$(echo "0.5*$(awk '/MemTotal/ { print $2 / (1024*1024)}' /proc/meminfo | cut -d'.' -f1)" | bc | xargs printf "%1.0f")
                 echo  "IBPS=$(echo \"0.5*$(awk '/MemTotal/ { print $2 / (1024*1024)}' /proc/meminfo | cut -d'.' -f1)\" | bc | xargs printf "%1.0f")" >> text.txt
				 sed -i "s/innodb_buffer_pool_size = 4G/innodb_buffer_pool_size = ${IBPS}G/" /etc/my.cnf
                 echo  "sed -i \"s/innodb_buffer_pool_size = 4G/innodb_buffer_pool_size = ${IBPS}G/\" /etc/my.cnf" >> text.txt
				 sed -i "s/innodb_buffer_pool_instances = 4/innodb_buffer_pool_instances = ${IBPS}/" /etc/my.cnf
                 echo  "sed -i \"s/innodb_buffer_pool_instances = 4/innodb_buffer_pool_instances = ${IBPS}/\" /etc/my.cnf" >> text.txt
				 
				 echo
                 YELLOWTXT "innodb_buffer_pool_size = ${IBPS}G"
				 echo  "innodb_buffer_pool_size = ${IBPS}G" >> text.txt
                 YELLOWTXT "innodb_buffer_pool_instances = ${IBPS}"
				 echo  "innodb_buffer_pool_instances = ${IBPS}" >> text.txt
				 
                echo
              echo
              ## get mysql tools
			  echo  "## get mysql tools" >> text.txt
              cd /usr/local/src
			  echo  "cd /usr/local/src" >> text.txt
              wget -qO - ${MYSQL_TOP} | tar -xzp && cd mytop*
			  echo  "wget -qO - ${MYSQL_TOP} | tar -xzp && cd mytop*" >> text.txt
              perl Makefile.PL && make && make install  >/dev/null 2>&1
			  echo  "perl Makefile.PL && make && make install  >/dev/null 2>&1" >> text.txt
              yum -y -q install percona-toolkit >/dev/null 2>&1
			  echo  "yum -y -q install percona-toolkit >/dev/null 2>&1" >> text.txt
              echo
              WHITETXT "Please use these tools to check and finetune your database:"
			  echo  "Please use these tools to check and finetune your database:" >> text.txt
...
echo  "---> Start Nginx (mainline) Repository installation? [y/n][n]:" >> text.txt

read repo_nginx_install
if [ "${repo_nginx_install}" == "y" ];then
          echo
            GREENTXT "Installation of Nginx (mainline) repository:"
			echo  "Installation of Nginx (mainline) repository:" >> text.txt
            echo
            WHITETXT "Downloading Nginx GPG key"
			echo  "Downloading Nginx GPG key" >> text.txt
            wget -qO /etc/pki/rpm-gpg/nginx_signing.key  http://nginx.org/packages/keys/nginx_signing.key
			echo  "wget -qO /etc/pki/rpm-gpg/nginx_signing.key  http://nginx.org/packages/keys/nginx_signing.key" >> text.txt
...
END" >> text.txt
            ...
            echo
            GREENTXT "REPOSITORY HAS BEEN INSTALLED  -  OK"
			echo  "REPOSITORY HAS BEEN INSTALLED  -  OK" >> text.txt
            echo
            GREENTXT "Installation of NGINX package:"
			echo  "Installation of NGINX package:" >> text.txt
            echo
            echo -n "     PROCESSING  "
            start_progress &
            pid="$!"
            yum -y -q install nginx nginx-module-geoip >/dev/null 2>&1
			echo  "yum -y -q install nginx nginx-module-geoip >/dev/null 2>&1" >> text.txt
            stop_progress "$pid"
            rpm  --quiet -q nginx
			echo  "rpm  --quiet -q nginx" >> text.txt
			
      if [ "$?" = 0 ]
        then
          echo
            GREENTXT "NGINX HAS BEEN INSTALLED  -  OK"
			echo  "NGINX HAS BEEN INSTALLED  -  OK" >> text.txt
            echo
            ## plug in service status alert
			echo  "## plug in service status alert" >> text.txt
            cp /usr/lib/systemd/system/nginx.service /etc/systemd/system/nginx.service
			echo  "cp /usr/lib/systemd/system/nginx.service /etc/systemd/system/nginx.service" >> text.txt
            sed -i "/^After.*/a OnFailure=service-status-mail@%n.service" /etc/systemd/system/nginx.service
			echo  "sed -i \"/^After.*/a OnFailure=service-status-mail@%n.service\" /etc/systemd/system/nginx.service" >> text.txt
            sed -i "/\[Install\]/i Restart=on-failure\nRestartSec=10\n" /etc/systemd/system/nginx.service
			echo  "sed -i \"/\[Install\]/i Restart=on-failure\nRestartSec=10\n\" /etc/systemd/system/nginx.service" >> text.txt
            sed -i "s,PIDFile=/run/nginx.pid,PIDFile=/var/run/nginx.pid," /etc/systemd/system/nginx.service
			echo  "sed -i \"s,PIDFile=/run/nginx.pid,PIDFile=/var/run/nginx.pid,\" /etc/systemd/system/nginx.service" >> text.txt
            sed -i "s/PrivateTmp=true/PrivateTmp=false/" /etc/systemd/system/nginx.service
			echo  "sed -i \"s/PrivateTmp=true/PrivateTmp=false/\" /etc/systemd/system/nginx.service" >> text.txt
            systemctl daemon-reload
			echo  "systemctl daemon-reload" >> text.txt
            systemctl enable nginx >/dev/null 2>&1
			echo  "systemctl enable nginx >/dev/null 2>&1" >> text.txt
...
echo  "---> Start the Remi repository and PHP 7.0 installation? [y/n][n]:" >> text.txt

read repo_remi_install
if [ "${repo_remi_install}" == "y" ];then
          echo
            GREENTXT "Installation of Remi repository:"
			echo  "Installation of Remi repository:" >> text.txt
            echo
            echo -n "     PROCESSING  "
            quick_progress &
            pid="$!"
            rpm --quiet -U ${REPO_REMI} >/dev/null 2>&1
			echo  "rpm --quiet -U ${REPO_REMI} >/dev/null 2>&1" >> text.txt
            stop_progress "$pid"
            rpm  --quiet -q remi-release
			echo  "rpm  --quiet -q remi-release" >> text.txt
      ...
      if [ "$?" = 0 ]
        then
          echo
			...
			echo  "Installation of PHP 7.0:" >> text.txt
            echo
            echo -n "     PROCESSING  "
            long_progress &
            pid="$!"
            yum --enablerepo=remi,remi-php70 -y -q install php ${PHP_PACKAGES[@]/#/php-} ${PHP_PECL_PACKAGES[@]/#/php-} >/dev/null 2>&1
            echo  "yum --enablerepo=remi,remi-php70 -y -q install php ${PHP_PACKAGES[@]/#/php-} ${PHP_PECL_PACKAGES[@]/#/php-} >/dev/null 2>&1" >> text.txt
			stop_progress "$pid"
            rpm  --quiet -q php
			echo  "rpm  --quiet -q php" >> text.txt
			
       if [ "$?" = 0 ]
         then
           echo
             GREENTXT "PHP HAS BEEN INSTALLED  -  OK"
			 echo  "PHP HAS BEEN INSTALLED  -  OK" >> text.txt
             ## plug in service status alert
			 echo  "## plug in service status alert" >> text.txt
             cp /usr/lib/systemd/system/php-fpm.service /etc/systemd/system/php-fpm.service
			 echo  "cp /usr/lib/systemd/system/php-fpm.service /etc/systemd/system/php-fpm.service" >> text.txt
             sed -i "s/PrivateTmp=true/PrivateTmp=false/" /etc/systemd/system/php-fpm.service
			 echo  "sed -i \"s/PrivateTmp=true/PrivateTmp=false/\" /etc/systemd/system/php-fpm.service" >> text.txt
             sed -i "/^After.*/a OnFailure=service-status-mail@%n.service" /etc/systemd/system/php-fpm.service
			 echo  "sed -i \"/^After.*/a OnFailure=service-status-mail@%n.service\" /etc/systemd/system/php-fpm.service" >> text.txt
             sed -i "/\[Install\]/i Restart=on-failure\nRestartSec=10\n" /etc/systemd/system/php-fpm.service
			 echo  "sed -i \"/\[Install\]/i Restart=on-failure\nRestartSec=10\n\" /etc/systemd/system/php-fpm.service" >> text.txt
             systemctl daemon-reload
			 echo  "systemctl daemon-reload" >> text.txt
             systemctl enable php-fpm >/dev/null 2>&1
			 echo  "systemctl enable php-fpm >/dev/null 2>&1" >> text.txt
             systemctl disable httpd >/dev/null 2>&1
			 echo  "systemctl disable httpd >/dev/null 2>&1" >> text.txt
             yum list installed | awk '/php.*x86_64/ {print "      ",$1}'
			 echo  "yum list installed | awk \'/php.*x86_64/ {print \"      \",$1}\'" >> text.txt
         ...
         echo
           echo
			...
			echo  "Installation of Redis, Memcached and Sphinx packages:" >> text.txt
            echo
            echo -n "     PROCESSING  "
            start_progress &
            pid="$!"
            yum --enablerepo=remi -y -q install redis memcached sphinx >/dev/null 2>&1
			echo  "yum --enablerepo=remi -y -q install redis memcached sphinx >/dev/null 2>&1" >> text.txt
            stop_progress "$pid"
            rpm  --quiet -q redis
			echo  "rpm  --quiet -q redis" >> text.txt
			
       if [ "$?" = 0 ]
         then
           echo
             GREENTXT "REDIS HAS BEEN INSTALLED"
			 echo  "REDIS HAS BEEN INSTALLED" >> text.txt
             systemctl disable redis >/dev/null 2>&1
			 echo  "systemctl disable redis >/dev/null 2>&1" >> text.txt
			 
             echo
for REDISPORT in 6379 6380
do
mkdir -p /var/lib/redis-${REDISPORT}
echo  "mkdir -p /var/lib/redis-${REDISPORT}" >> text.txt
chmod 755 /var/lib/redis-${REDISPORT}
echo  "chmod 755 /var/lib/redis-${REDISPORT}" >> text.txt
chown redis /var/lib/redis-${REDISPORT}
echo  "chown redis /var/lib/redis-${REDISPORT}" >> text.txt
\cp -rf /etc/redis.conf /etc/redis-${REDISPORT}.conf
echo  "\cp -rf /etc/redis.conf /etc/redis-${REDISPORT}.conf" >> text.txt
\cp -rf /usr/lib/systemd/system/redis.service /etc/systemd/system/redis-${REDISPORT}.service
echo  "\cp -rf /usr/lib/systemd/system/redis.service /etc/systemd/system/redis-${REDISPORT}.service" >> text.txt
sed -i "s/daemonize no/daemonize yes/"  /etc/redis-${REDISPORT}.conf
echo  "sed -i \"s/daemonize no/daemonize yes/\"  /etc/redis-${REDISPORT}.conf" >> text.txt
sed -i "s/^bind 127.0.0.1.*/bind 127.0.0.1/"  /etc/redis-${REDISPORT}.conf
echo  "sed -i \"s/^bind 127.0.0.1.*/bind 127.0.0.1/\"  /etc/redis-${REDISPORT}.conf" >> text.txt
sed -i "s/^dir.*/dir \/var\/lib\/redis-${REDISPORT}\//"  /etc/redis-${REDISPORT}.conf
echo  "sed -i \"s/^dir.*/dir \/var\/lib\/redis-${REDISPORT}\//\"  /etc/redis-${REDISPORT}.conf" >> text.txt
sed -i "s/^logfile.*/logfile \/var\/log\/redis\/redis-${REDISPORT}.log/"  /etc/redis-${REDISPORT}.conf
echo  "sed -i \"s/^logfile.*/logfile \/var\/log\/redis\/redis-${REDISPORT}.log/\"  /etc/redis-${REDISPORT}.conf" >> text.txt
sed -i "s/^pidfile.*/pidfile \/var\/run\/redis\/redis-${REDISPORT}.pid/"  /etc/redis-${REDISPORT}.conf
echo  "sed -i \"s/^pidfile.*/pidfile \/var\/run\/redis\/redis-${REDISPORT}.pid/\"  /etc/redis-${REDISPORT}.conf" >> text.txt
sed -i "s/^port.*/port ${REDISPORT}/" /etc/redis-${REDISPORT}.conf
echo  "sed -i \"s/^port.*/port ${REDISPORT}/\" /etc/redis-${REDISPORT}.conf" >> text.txt
sed -i "s/redis.conf/redis-${REDISPORT}.conf/" /etc/systemd/system/redis-${REDISPORT}.service
echo  "sed -i \"s/redis.conf/redis-${REDISPORT}.conf/\" /etc/systemd/system/redis-${REDISPORT}.service" >> text.txt
sed -i "/^After.*/a OnFailure=service-status-mail@%n.service" /etc/systemd/system/redis-${REDISPORT}.service
echo  "sed -i \"/^After.*/a OnFailure=service-status-mail@%n.service\" /etc/systemd/system/redis-${REDISPORT}.service" >> text.txt
sed -i "/\[Install\]/i Restart=on-failure\nRestartSec=10\n" /etc/systemd/system/redis-${REDISPORT}.service
echo  "sed -i \"/\[Install\]/i Restart=on-failure\nRestartSec=10\n\" /etc/systemd/system/redis-${REDISPORT}.service" >> text.txt
...
OPTIONS="-l 127.0.0.1"
END
echo  "cat > /etc/sysconfig/memcached <<END
PORT=\"11211\"
USER=\"memcached\"
MAXCONN=\"5024\"
CACHESIZE=\"128\"
...
echo  "cp /usr/lib/systemd/system/memcached.service /etc/systemd/system/memcached.service" >> text.txt
cp /usr/lib/systemd/system/searchd.service /etc/systemd/system/searchd.service
echo  "cp /usr/lib/systemd/system/searchd.service /etc/systemd/system/searchd.service" >> text.txt
sed -i "/^After.*/a OnFailure=service-status-mail@%n.service" /etc/systemd/system/memcached.service
echo  "sed -i \"/^After.*/a OnFailure=service-status-mail@%n.service\" /etc/systemd/system/memcached.service" >> text.txt
sed -i "/\[Install\]/i Restart=on-failure\nRestartSec=10\n" /etc/systemd/system/memcached.service
echo  "sed -i \"/\[Install\]/i Restart=on-failure\nRestartSec=10\n\" /etc/systemd/system/memcached.service" >> text.txt
sed -i "/^After.*/a OnFailure=service-status-mail@%n.service" /etc/systemd/system/searchd.service
echo  "sed -i \"/^After.*/a OnFailure=service-status-mail@%n.service\" /etc/systemd/system/searchd.service" >> text.txt
sed -i "/\[Install\]/i Restart=on-failure\nRestartSec=10\n" /etc/systemd/system/searchd.service
echo  "sed -i \"/\[Install\]/i Restart=on-failure\nRestartSec=10\n\" /etc/systemd/system/searchd.service" >> text.txt
systemctl daemon-reload
echo  "systemctl daemon-reload" >> text.txt
systemctl enable redis-6379 >/dev/null 2>&1
echo  "systemctl enable redis-6379 >/dev/null 2>&1" >> text.txt
systemctl enable redis-6380 >/dev/null 2>&1
echo  "systemctl enable redis-6380 >/dev/null 2>&1" >> text.txt
systemctl enable memcached  >/dev/null 2>&1
echo  "systemctl enable memcached  >/dev/null 2>&1" >> text.txt
                else
               echo
...
echo  "---> Start Varnish 4.x installation? [y/n][n]:" >> text.txt
read varnish_install
if [ "${varnish_install}" == "y" ];then
          echo
            GREENTXT "Installation of Varnish package:"
			echo  "Installation of Varnish package:" >> text.txt
            echo
            rpm --quiet --nosignature -i https://repo.varnish-cache.org/redhat/varnish-4.1.el7.rpm >/dev/null 2>&1
			echo  "rpm --quiet --nosignature -i https://repo.varnish-cache.org/redhat/varnish-4.1.el7.rpm >/dev/null 2>&1" >> text.txt
            echo -n "     PROCESSING  "
            start_progress &
            pid="$!"
            yum -y -q install varnish >/dev/null 2>&1
			echo  "yum -y -q install varnish >/dev/null 2>&1" >> text.txt
            stop_progress "$pid"
            rpm  --quiet -q varnish
			echo  "rpm  --quiet -q varnish" >> text.txt
			
      if [ "$?" = 0 ]
        then
          echo
	    wget -qO /etc/systemd/system/varnish.service ${REPO_MASCM_TMP}varnish.service
		echo  "wget -qO /etc/systemd/system/varnish.service ${REPO_MASCM_TMP}varnish.service" >> text.txt
            wget -qO /etc/varnish/varnish.params ${REPO_MASCM_TMP}varnish.params
			echo  "wget -qO /etc/varnish/varnish.params ${REPO_MASCM_TMP}varnish.params" >> text.txt
            systemctl daemon-reload >/dev/null 2>&1
			echo  "systemctl daemon-reload >/dev/null 2>&1" >> text.txt
            GREENTXT "VARNISH HAS BEEN INSTALLED  -  OK"
			echo  "VARNISH HAS BEEN INSTALLED  -  OK" >> text.txt
               else
              echo
...
echo  "---> Start HHVM installation? [y/n][n]:" >> text.txt
read hhvm_install
...
echo
			...
			echo  "Installation of HHVM package:" >> text.txt
            echo
            echo -n "     PROCESSING  "
            start_progress &
            pid="$!"
            yum -y -q install hhvm >/dev/null 2>&1
			echo  "yum -y -q install hhvm >/dev/null 2>&1" >> text.txt
            stop_progress "$pid"
            rpm  --quiet -q hhvm
			echo  "rpm  --quiet -q hhvm" >> text.txt
			
      if [ "$?" = 0 ]
        then
          echo
            GREENTXT "HHVM HAS BEEN INSTALLED  -  OK"
			echo  "HHVM HAS BEEN INSTALLED  -  OK" >> text.txt
            echo
            ## plug in service status alert
			echo  "## plug in service status alert" >> text.txt
            cp /usr/lib/systemd/system/hhvm.service /etc/systemd/system/hhvm.service
			echo  "cp /usr/lib/systemd/system/hhvm.service /etc/systemd/system/hhvm.service" >> text.txt
            sed -i "/^Description=.*/a OnFailure=service-status-mail@%n.service" /etc/systemd/system/hhvm.service
			echo  "sed -i \"/^Description=.*/a OnFailure=service-status-mail@%n.service\" /etc/systemd/system/hhvm.service" >> text.txt
            sed -i "/\[Install\]/i Restart=on-failure\nRestartSec=10\n" /etc/systemd/system/hhvm.service
			echo  "sed -i \"/\[Install\]/i Restart=on-failure\nRestartSec=10\n\" /etc/systemd/system/hhvm.service" >> text.txt
            systemctl daemon-reload
			echo  "systemctl daemon-reload" >> text.txt
            systemctl enable hhvm >/dev/null 2>&1
			echo  "systemctl enable hhvm >/dev/null 2>&1" >> text.txt
...
echo "-------------------------------------------------------------------------------------"
echo  "| THE INSTALLATION OF REPOSITORIES AND PACKAGES IS COMPLETE |" >> text.txt

echo
echo
GREENTXT "NOW WE ARE GOING TO CONFIGURE EVERYTHING"
echo  "NOW WE ARE GOING TO CONFIGURE EVERYTHING" >> text.txt
echo
pause "---> Press [Enter] key to proceed"
echo
echo "Load optimized configs of php, opcache, fpm, fastcgi, sysctl, varnish"
echo  "Load optimized configs of php, opcache, fpm, fastcgi, sysctl, varnish" >> text.txt
...
sysctl -q -p
echo  "sysctl -q -p" >> text.txt

echo
WHITETXT "sysctl.conf loaded ${GREEN} [ok]"
echo  "sysctl.conf loaded ${GREEN} [ok]" >> text.txt
...
echo  "cat > /etc/php.d/10-opcache.ini <<END
zend_extension=opcache.so
opcache.enable = 1
opcache.enable_cli = 1
opcache.memory_consumption = 256
...
echo  "opcache.ini loaded ${GREEN} [ok]" >> text.txt

#Tweak php.ini.
echo  "#Tweak php.ini." >> text.txt

cp /etc/php.ini /etc/php.ini.BACK
echo  "cp /etc/php.ini /etc/php.ini.BACK" >> text.txt
sed -i 's/^\(max_execution_time = \)[0-9]*/\17200/' /etc/php.ini
echo  "sed -i 's/^\(max_execution_time = \)[0-9]*/\17200/' /etc/php.ini" >> text.txt
sed -i 's/^\(max_input_time = \)[0-9]*/\17200/' /etc/php.ini
echo  "sed -i 's/^\(max_input_time = \)[0-9]*/\17200/' /etc/php.ini" >> text.txt
sed -i 's/^\(memory_limit = \)[0-9]*M/\11024M/' /etc/php.ini
echo  "sed -i 's/^\(memory_limit = \)[0-9]*M/\11024M/' /etc/php.ini" >> text.txt
sed -i 's/^\(post_max_size = \)[0-9]*M/\164M/' /etc/php.ini
echo  "sed -i 's/^\(post_max_size = \)[0-9]*M/\164M/' /etc/php.ini" >> text.txt
sed -i 's/^\(upload_max_filesize = \)[0-9]*M/\164M/' /etc/php.ini
echo  "sed -i 's/^\(upload_max_filesize = \)[0-9]*M/\164M/' /etc/php.ini" >> text.txt
sed -i 's/expose_php = On/expose_php = Off/' /etc/php.ini
echo  "sed -i 's/expose_php = On/expose_php = Off/' /etc/php.ini" >> text.txt
sed -i 's/;realpath_cache_size = 16k/realpath_cache_size = 512k/' /etc/php.ini
echo  "sed -i 's/;realpath_cache_size = 16k/realpath_cache_size = 512k/' /etc/php.ini" >> text.txt
sed -i 's/;realpath_cache_ttl = 120/realpath_cache_ttl = 86400/' /etc/php.ini
echo  "sed -i 's/;realpath_cache_ttl = 120/realpath_cache_ttl = 86400/' /etc/php.ini" >> text.txt
sed -i 's/short_open_tag = Off/short_open_tag = On/' /etc/php.ini
echo  "sed -i 's/short_open_tag = Off/short_open_tag = On/' /etc/php.ini" >> text.txt
sed -i 's/; max_input_vars = 1000/max_input_vars = 50000/' /etc/php.ini
echo  "sed -i 's/; max_input_vars = 1000/max_input_vars = 50000/' /etc/php.ini" >> text.txt
sed -i 's/session.gc_maxlifetime = 1440/session.gc_maxlifetime = 28800/' /etc/php.ini
echo  "sed -i 's/session.gc_maxlifetime = 1440/session.gc_maxlifetime = 28800/' /etc/php.ini" >> text.txt
sed -i 's/mysql.allow_persistent = On/mysql.allow_persistent = Off/' /etc/php.ini
echo  "sed -i 's/mysql.allow_persistent = On/mysql.allow_persistent = Off/' /etc/php.ini" >> text.txt
sed -i 's/mysqli.allow_persistent = On/mysqli.allow_persistent = Off/' /etc/php.ini
echo  "sed -i 's/mysqli.allow_persistent = On/mysqli.allow_persistent = Off/' /etc/php.ini" >> text.txt
sed -i 's/pm = dynamic/pm = ondemand/' /etc/php-fpm.d/www.conf
echo  "sed -i 's/pm = dynamic/pm = ondemand/' /etc/php-fpm.d/www.conf" >> text.txt
sed -i 's/;pm.max_requests = 500/pm.max_requests = 10000/' /etc/php-fpm.d/www.conf
echo  "sed -i 's/;pm.max_requests = 500/pm.max_requests = 10000/' /etc/php-fpm.d/www.conf" >> text.txt
sed -i 's/pm.max_children = 50/pm.max_children = 1000/' /etc/php-fpm.d/www.conf
echo  "sed -i 's/pm.max_children = 50/pm.max_children = 1000/' /etc/php-fpm.d/www.conf" >> text.txt


WHITETXT "php.ini loaded ${GREEN} [ok]"
echo  "php.ini loaded ${GREEN} [ok]" >> text.txt
echo
echo "*         soft    nofile          700000" >> /etc/security/limits.conf
echo  "echo \"*         soft    nofile          700000\" >> /etc/security/limits.conf" >> text.txt
echo "*         hard    nofile          1000000" >> /etc/security/limits.conf
echo  "echo \"*         hard    nofile          1000000\" >> /etc/security/limits.conf" >> text.txt
echo
echo
echo "-------------------------------------------------------------------------------------"
BLUEBG "| FINISHED PACKAGES INSTALLATION |"
echo "-------------------------------------------------------------------------------------"
echo  "| FINISHED PACKAGES INSTALLATION |" >> text.txt
echo
echo
